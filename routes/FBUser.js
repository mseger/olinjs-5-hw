...
exports.login = function (req, res) {
  req.facebook.api('/me', function(err, data){
  	req.facebook.api('/me/picture?redirect=false&type=large', function(err, picData){
  		if(FBUser.findOne({name: data.name})){
  			var currUser = FBUser.findOne({name: data.name}, function (err, user){
  				console.log("Found user: ", user); 
  				req.session.user = user;
  				res.redirect('/homepage');
  			});
  		}else{
  			var loggedInUser = new FBUser({name: data.name, profPicURL: picData.data.url, profileBackground: "white"});
